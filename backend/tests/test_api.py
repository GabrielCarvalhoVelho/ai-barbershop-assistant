...
class TestRoot:
    def test_returns_running_message(self):
        response = client.get("/")
...
class TestChatSuccess:
    def test_valid_message(self):
        response = client.post("/chat", json={"message": "Quero agendar um corte"})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_sanitizes_spaces(self):
        response = client.post("/chat", json={"message": "quero   agendar    corte"})
        assert response.status_code == 200
...
class TestChatValidation422:
    ...
    def test_single_repeated_char(self):
        response = client.post("/chat", json={"message": "aaaaaaa"})
        assert response.status_code == 422
        data = response.json()
    ...
    def test_no_details_null_in_response(self):
...
class TestChatBusiness400:
    def test_repeated_chars_spam(self):
        response = client.post(
            "/chat", json={"message": "olha isso aaaaaaaaaa que legal"}
        )
    ...
    def test_repeated_word_spam(self):
        response = client.post(
            "/chat", json={"message": "spam spam spam spam spam"}
        )
        assert response.status_code == 400
        data = response.json()
        assert "repetitivo" in data["error"]
