...
class TestAuthenticateUser:
    ...
    async def test_success_returns_user(self):
        ...
        result = await authenticate_user("user@test.com", "correct_pass", repo)
    ...
    async def test_wrong_password_raises(self):
        ...
        with pytest.raises(AuthenticationError):
            await authenticate_user("user@test.com", "wrong_pass", repo)
    ...
    async def test_user_not_found_raises(self):
        ...
        with pytest.raises(AuthenticationError):
            await authenticate_user("nobody@test.com", "pass", repo)
    ...
    async def test_inactive_user_raises(self):
        ...
        with pytest.raises(AuthenticationError):
            await authenticate_user("user@test.com", "correct_pass", repo)
    ...
    async def test_same_error_message_prevents_enumeration(self):
        ...
        repo_not_found.get_by_email.return_value = None

        repo_wrong_pass = AsyncMock()
        repo_wrong_pass.get_by_email.return_value = _make_user()

        with pytest.raises(AuthenticationError) as exc_not_found:
            await authenticate_user("nobody@test.com", "pass", repo_not_found)

        with pytest.raises(AuthenticationError) as exc_wrong_pass:
            await authenticate_user("user@test.com", "wrong", repo_wrong_pass)
